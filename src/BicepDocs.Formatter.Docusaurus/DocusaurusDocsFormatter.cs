...
public class DocusaurusDocsFormatter : IDocsFormatter
{
    ...
    public async Task<IImmutableList<GenerationFile>> GenerateModuleDocs(FormatterContext context)
    {
        ...
        var configuration =
            _configurationLoader.GetFormatterOptionsOrDefault<DocusaurusOptions>(context.FormatterOptions, Formatter);
        ...
        {
            var folderPath = Path.GetDirectoryName(markdownFile.FilePath);
            var prevPath = "";
            ...
            var pathElements = folderPath.Split('/').Where(x => !string.IsNullOrEmpty(x)).ToArray();
